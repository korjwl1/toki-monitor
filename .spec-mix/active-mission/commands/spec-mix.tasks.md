...
1. **Setup**: Run `.spec-mix/scripts/bash/check-prerequisites.sh --json` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. All paths must be absolute. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").
...
4. **Generate tasks.md**: Use `.spec-mix.spec-mix/templates/tasks-template.md` as structure, fill with:
...
5. **MANDATORY: Generate Work Package files with Lane Structure**:
   ...
   - For each task in tasks.md, create a Work Package file in `planned/`:
     ...
     - Use `.spec-mix/active-mission.spec-mix/templates/work-package-template.md` as template
...
6. **MANDATORY Workflow Instructions**:
   ...
   2. MOVE: bash .spec-mix.spec-mix/scripts/move-task.sh WP## planned doing [FEATURE_DIR]
   3. IMPLEMENT: Write code (commits MUST include [WP##])
   4. COMPLETE: bash .spec-mix.spec-mix/scripts/move-task.sh WP## doing for_review [FEATURE_DIR]
