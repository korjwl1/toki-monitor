...
struct PanelOptionsTests {
    ...
    @Test("each threshold becomes a band running to the next one")
    func bandsSpanToTheNextThreshold() {
        let scale = GaugePanelView.Scale(min: 0, max: 100, fraction: 0.5)
        let bands = GaugePanelView.bands(scale: scale, thresholds: [
            ThresholdStep(value: 80, color: "red"),
            ThresholdStep(value: 50, color: "orange"),
        ])
        #expect(bands.count == 2)
        #expect(bands[0].start == 0.5)
        #expect(bands[0].end == 0.8)
        #expect(bands[1].start == 0.8)
        #expect(bands[1].end == 1.0, "the last band runs to the end of the scale")
    }

    @Test("a threshold outside the scale colours the end rather than vanishing")
    func outOfRangeThresholdIsClamped() {
        let scale = GaugePanelView.Scale(min: 0, max: 100, fraction: 0.5)
        let bands = GaugePanelView.bands(scale: scale, thresholds: [
            ThresholdStep(value: 200, color: "red"),
        ])
        #expect(bands.isEmpty, "a band with no width on the dial is not drawn")
    }

    @Test("switching threshold markers off leaves the value its own colour")
    func thresholdMarkersOffMeansNoThresholdColour() {
        var options = PanelDisplayOptions()
        options.thresholds = [ThresholdStep(value: 10, color: "red")]
        ...
        #expect(GaugePanelView.valueColor(90, options: options) != .accentColor)
        #expect(GaugePanelView.valueColor(1, options: options) == .accentColor,
                "a value below every threshold has reached none of them")
    }
