...
final class DashboardConfigStore {
    ...
    // MARK: - File Import/Export state
    //
    // The pickers themselves live in Presentation (AppKit is not Domain's to
    // import); these stay here because an extension cannot hold stored
    // properties and because they are part of the store's observable state.

    /// Set when `exportToFile` fails; nil after a success or a cancel.
    internal(set) var lastExportError: String?

    /// Set when `chooseImportFile` fails; nil after a success or a cancel.
    internal(set) var lastImportError: String?
}
